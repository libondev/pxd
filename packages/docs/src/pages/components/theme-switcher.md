...
;(() => {
  ...
  let p = matchMedia('(prefers-color-scheme:dark)').matches
  let s = localStorage.getItem('fe.system.color-scheme') || 'auto'
  if (s === 'dark' || (p && s === 'auto')) document.documentElement.classList.toggle('dark', true)
})()
```
