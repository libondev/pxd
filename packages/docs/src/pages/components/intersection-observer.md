...
<template>
  <div>
    <span>Now, content is </span>
    <span class="font-semibold underline" :class="isVisible ? 'text-blue-900' : 'text-red-900'">{{ isVisible ? 'visible' : 'hidden' }}</span>
  </div>

  <div class="w-100 h-60 overflow-auto mt-2 p-4 bg-background-secondary">
    <PIntersectionObserver
      class="bg-background w-80 h-40 rounded-md border border-dashed mb-60 mr-100"
